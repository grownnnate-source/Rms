import { IceCream, CreditCard, LayoutDashboard, Clock, RotateCcw, PlusCircle } from "lucide-react";
export const HeaderNav = ({
  ...
  return <header className="no-print sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-[#5A3E36]/10 px-4 lg:px-6 py-2.5 shadow-sm">
    ...
    className={`flex items-center gap-2 px-3.5 py-2 rounded-lg text-sm font-semibold transition-all duration-200 cursor-pointer ${currentRole === "server" ? "bg-white text-[#5A3E36] shadow-sm font-bold border border-[#5A3E36]/10" : "text-[#78716C] hover:text-[#5A3E36]"}`}
  >
    ...
    className={`flex items-center gap-2 px-3.5 py-2 rounded-lg text-sm font-semibold transition-all duration-200 relative cursor-pointer ${currentRole === "cashier" ? "bg-white text-[#5A3E36] shadow-sm font-bold border border-[#5A3E36]/10" : "text-[#78716C] hover:text-[#5A3E36]"}`}
  >
            <CreditCard className={`w-4 h-4 ${currentRole === "cashier" ? "text-[#E85D75]" : "text-[#78716C]"}`} />
            <span>2. Cashier</span>
            {pendingOrdersCount > 0 && <span className="inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold leading-none text-white bg-[#E85D75] rounded-full animate-bounce">
                {pendingOrdersCount}
              </span>}
          </button>
    ...
    className={`flex items-center gap-2 px-3.5 py-2 rounded-lg text-sm font-semibold transition-all duration-200 cursor-pointer ${currentRole === "manager" ? "bg-white text-[#5A3E36] shadow-sm font-bold border border-[#5A3E36]/10" : "text-[#78716C] hover:text-[#5A3E36]"}`}
  >
