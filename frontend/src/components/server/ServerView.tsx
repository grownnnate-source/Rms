...
interface ServerViewProps {
  ...
  onClearCart: () => void;
  onSubmitOrder: (customerNote?: string) => Order | null;
  recentOrders: Order[];
...
export const ServerView: React.FC<ServerViewProps> = ({
  ...
  const [selectedCategory, setSelectedCategory] = useState<ProductCategory>('ice_cream');
  ...
  // University shop student flat tax 5% or 0
  ...
  const handleProductCardClick = (product: Product) => {
    ...
    // For ice creams, always show customization modal (scoops, serving, toppings)
    if (product.category === 'ice_cream') {
      setActiveCustomizingProduct(product);
    } else {
      // Direct fast-add for toppings, cones, cups, drinks (or customize if desired)
              ...
              return (
                <div
                  ...
                  className={`relative bg-white rounded-2xl p-4 border transition-all duration-200 flex flex-col justify-between ${
                    product.available
                      ? 'border-[#5A3E36]/10 shadow-xs hover:shadow-md hover:border-[#5A3E36]/25 group'
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2.5">
                    ...
                    <div className="flex flex-col items-end">
                      {product.badge && (
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#F58FA3]/20 text-[#5A3E36] border border-[#F58FA3]/30 mb-1">
                          {product.badge}
                        </span>
                      )}
                      {!product.available && (
