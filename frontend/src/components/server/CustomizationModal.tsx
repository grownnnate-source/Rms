...
import { Product, OrderItem, ServingType } from '../../types';
...
export const CustomizationModal: React.FC<CustomizationModalProps> = ({
  ...
  const [scoops, setScoops] = useState<number>(product.scoopsDefault || 1);
  const [serving, setServing] = useState<ServingType>('Cup');
  const [selectedToppings, setSelectedToppings] = useState<string[]>([]);
  const [quantity, setQuantity] = useState<number>(1);

  // Extra scoops calculation (+35 ETB per extra scoop over 1)
  const extraScoopPrice = isIceCream && scoops > 1 ? (scoops - 1) * 35 : 0;

  // Serving upcharge if Waffle Bowl (+20 ETB) or Cone (+10 ETB)
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-sm font-bold text-[#5A3E36] flex items-center gap-1.5">
                  <span>Scoops</span>
                  {scoops > 1 && (
                    <span className="text-xs font-normal text-[#E85D75]">(+35 ETB each extra)</span>
                  )}
                </label>
          ...
          {isIceCream && (
            <div>
              ...
              <div className="grid grid-cols-3 gap-2.5">
                {(['Cup', 'Cone', 'Waffle Bowl'] as ServingType[]).map((type) => {
