...
type ManagerTab = 'dashboard' | 'orders' | 'products' | 'expenses' | 'analytics' | 'settings';
...
export const ManagerView: React.FC<ManagerViewProps> = ({
  ...
  const [salesPeriod, setSalesPeriod] = useState<'Daily' | 'Monthly' | 'Yearly'>('Daily');
  ...
  const productSalesMap: Record<string, { name: string; category: string; sold: number; revenue: number }> = {};
  ...
  // Fallback defaults if few orders paid yet
  ...
  const activeChartData = 
    salesPeriod === 'Daily' ? dailySales : 
    salesPeriod === 'Monthly' ? monthlySales : yearlySales;

  const activeAnalyticsData =
    analyticsPeriod === 'Daily' ? dailySales :
    analyticsPeriod === 'Monthly' ? monthlySales : yearlySales;
  ...
  const averageOrderValue = currentAnalyticsOrders > 0 
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                      isActive
                        ? 'bg-[#5A3E36] text-[#FFF9F2] shadow-sm'
                        : 'text-[#78716C] hover:text-[#5A3E36] hover:bg-white/80'
                    }`}
                  >
                    ...
                    {tab.count !== undefined && (
                      <span className={`text-[10px] font-mono px-1.5 py-0.2 rounded-full ${
                        isActive ? 'bg-white/20 text-white' : 'bg-[#5A3E36]/10 text-[#5A3E36]'
                      }`}>
                        {tab.count}
                      </span>
                    )}
                          ...
                          return (
                            <tr 
                              ...
                              className={`hover:bg-stone-50 transition-colors ${
                                !prod.available ? 'opacity-65 bg-stone-50/70' : ''
                              }`}
                            >
                              <td className="px-4 py-3">
                                <ProductIcon
                              ...
                              <td className="px-4 py-3">
                                ...
                                {prod.badge && (
                                  <span className="inline-block mt-0.5 text-[9px] font-bold px-1.5 py-0.2 rounded bg-[#F58FA3]/20 text-[#5A3E36]">
                                    {prod.badge}
                                  </span>
                                )}
                              </td>
                              ...
                              <td className="px-4 py-3">
                                <button
                                  ...
                                  className={`px-2.5 py-1 rounded-full text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                                    prod.available
                                      ? 'bg-[#65A30D]/15 text-[#4D7C0F] hover:bg-[#65A30D]/25'
                                      : 'bg-stone-200 text-stone-600 hover:bg-stone-300'
                                  }`}
                                  title="Click to toggle availability"
                                >
                                  <span className={`w-1.5 h-1.5 rounded-full ${prod.available ? 'bg-[#65A30D]' : 'bg-stone-400'}`}></span>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-5 rounded-2xl border border-[#5A3E36]/10 shadow-xs space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                    {staff.map((member) => (
                      <div key={member.id} className="p-4 rounded-xl bg-[#FFF9F2] border border-[#5A3E36]/10 space-y-3">
                        <div className="flex items-center justify-between">
