...
export const ProductModal: React.FC<ProductModalProps> = ({
  ...
  const [category, setCategory] = useState<ProductCategory>(initialProduct?.category || 'ice_cream');
  const [description, setDescription] = useState<string>(initialProduct?.description || '');
  const [price, setPrice] = useState<number>(initialProduct?.price || 100);
  const [available, setAvailable] = useState<boolean>(initialProduct ? initialProduct.available : true);
  const [iconName, setIconName] = useState<string>(initialProduct?.iconName || 'IceCream');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const productToSave: Product = {
      ...
      colorAccent: category === 'ice_cream' ? '#5A3E36' : undefined,
