...
export const CategoryTabs = ({ categories = CATEGORIES, selectedCategory, onSelectCategory, products = [] }) => (
      ...
      return (
        <button
          ...
          className={`px-4 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-all duration-200 flex items-center gap-2 cursor-pointer ${
            isActive ? "bg-[#5A3E36] text-[#FFF9F2] shadow-sm ring-2 ring-[#5A3E36]/20" : "bg-white text-[#78716C] hover:text-[#5A3E36] hover:bg-stone-50 border border-[#5A3E36]/10"
          }`}
        >
          <span>{cat.label}</span>
