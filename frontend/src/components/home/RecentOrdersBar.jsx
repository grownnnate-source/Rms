  ...
  <div className="bg-white rounded-2xl p-4 border border-[#5A3E36]/10 shadow-xs">
    <div className="flex items-center justify-between mb-3">
      <div className="flex items-center gap-2">
        ...
        <h4 className="text-xs font-bold uppercase tracking-wider text-[#5A3E36]">Recent Server Submissions</h4>
      </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
      {recentOrders.slice(0, 3).map((ord) => (
        <div key={ord.id} className="p-2.5 rounded-xl bg-[#FFF9F2] border border-[#5A3E36]/10 flex items-center justify-between text-xs">
          <div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
              ord.status === "PAID" ? "bg-[#65A30D]/15 text-[#4D7C0F]" : ord.status === "PENDING" ? "bg-amber-100 text-amber-800 animate-pulse" : "bg-stone-200 text-stone-600"
            }`}>
              {ord.status}
            </span>
          </div>
