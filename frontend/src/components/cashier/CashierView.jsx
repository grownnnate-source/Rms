...
export const CashierView = ({
  ...
  const filteredOrders = orders.filter((ord) => {
    ...
    const matchesSearch = ord.orderNumber.toString().includes(searchQuery) || ord.serverName.toLowerCase().includes(searchQuery.toLowerCase()) || ord.items.some((i) => i.name.toLowerCase().includes(searchQuery.toLowerCase()));
    return matchesStatus && matchesSearch;
  });
  const pendingCount = orders.filter((o) => o.status === "PENDING").length;
  const paidCount = orders.filter((o) => o.status === "PAID").length;
  const totalSettledToday = orders.filter((o) => o.status === "PAID").reduce((sum, o) => sum + o.total, 0);
  const getStatusPill = (status) => {
    switch (status) {
      case "PAID":
        return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-[#65A30D]/15 text-[#4D7C0F] border border-[#65A30D]/30">
            <CheckCircle2 className="w-3 h-3 text-[#65A30D]" />
            <span>PAID</span>
          </span>;
      case "FAILED":
        return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-700 border border-rose-200">
            <AlertCircle className="w-3 h-3 text-rose-600" />
            <span>FAILED</span>
          </span>;
      case "EXPIRED":
        return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-stone-200 text-stone-700 border border-stone-300">
            <Clock className="w-3 h-3 text-stone-500" />
            <span>EXPIRED</span>
          </span>;
      case "PENDING":
      default:
        return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-800 border border-amber-300">
            <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-ping" />
            <span>PAYMENT PENDING</span>
          </span>;
    }
  };
  return <div className="max-w-7xl mx-auto p-3 sm:p-4 lg:p-6 pb-20 space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white p-4 rounded-2xl border border-[#5A3E36]/10 shadow-xs">
          ...
          <div className="text-2xl font-black text-[#5A3E36] mt-1 flex items-center gap-2">
            ...
            {pendingCount > 0 && <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 animate-pulse">
                Action Required
              </span>}
          </div>
        </div>
    ...
    return <button
      ...
      className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all cursor-pointer flex items-center gap-1.5 ${isActive ? "bg-[#5A3E36] text-white shadow-xs" : "bg-[#FFF9F2] text-[#78716C] hover:text-[#5A3E36] border border-[#5A3E36]/10"}`}
    >
                <span>{f.label}</span>
  ...
  />
    ...
    className={`p-1.5 rounded-lg transition-all cursor-pointer ${viewMode === "cards" ? "bg-white text-[#5A3E36] shadow-2xs" : "text-[#78716C]"}`}
    title="Cards Layout"
  >
    ...
    className={`p-1.5 rounded-lg transition-all cursor-pointer ${viewMode === "table" ? "bg-white text-[#5A3E36] shadow-2xs" : "text-[#78716C]"}`}
  ...
  >
    ...
    return <div
      ...
      className={`bg-white rounded-2xl border p-5 shadow-xs transition-all flex flex-col justify-between ${isPending ? "border-amber-300 ring-1 ring-amber-200/60" : isPaid ? "border-[#65A30D]/30" : "border-stone-200"}`}
    >
                {
      /* Card Top Row */
    }
                  ...
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {isPending && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-amber-100 text-amber-800">
                            Action
                          </span>}
                      </div>
      ...
      /* Items list breakdown */
                  ...
                  <div className="bg-[#FFF9F2] p-3 rounded-xl border border-[#5A3E36]/10 space-y-1.5 mb-4">
                    {order.items.map((item, idx) => <div key={idx} className="text-xs text-[#292524] flex items-start justify-between">
                        <div>
      ...
      /* Toppings / scoops notes */
                          ...
                          {(item.scoops > 0 || item.toppings.length > 0) && <div className="text-[11px] text-stone-500 pl-2">
                    ...
                    {order.customerNote && <div className="pt-1.5 mt-1 border-t border-[#5A3E36]/10 text-[11px] text-[#78716C] italic">
    ...
    <div className="bg-white rounded-2xl border border-[#5A3E36]/10 shadow-xs overflow-hidden">
      ...
      const isPending = order.status === "PENDING";
      const isPaid = order.status === "PAID";
      return <tr key={order.id} className="hover:bg-stone-50/70 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-stone-700 max-w-xs">
                        <div className="truncate">
                          {order.items.map((i) => `${i.name} \xD7 ${i.quantity}`).join(", ")}
                        </div>
                      </td>
      ...
      {filteredOrders.length === 0 && <div className="bg-white rounded-2xl p-12 text-center border border-stone-200">
    ...
    /* Chapa Payment Request Modal */
      ...
      {selectedPaymentOrder && <ChapaPaymentModal
